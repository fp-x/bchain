...
const isString = (val) => typeof val === 'string' || ((!!val && typeof val === 'object') && Object.prototype.toString.call(val) === '[object String]');
...
export class BChain {
	...
	constructor (public auth, public url = 'http://localhost:2890/') {
		this.undefinedFunctions = Object.keys(this.commands).map(method => {
			...
			const commands: [object|string] = this.commands[method];
	...
	private parseParams (method, args): any[] {
		const commandParams = this.commands[method];
	...
	private getFunctionString(method: string, params: [object|string]) {
		const defaultToStr = (d) => isString(d)? `'${d}'` : (Array.isArray(d)? '[]' : (isObject(d)? '{}' : d));
		const getDefaultStr = (param) => (param[Object.keys(param)[0]] === undefined)? '?': ' = '+defaultToStr(param[Object.keys(param)[0]]);
		const fixStr = (param: string, addDefault = false) => param.toString().replace(/-/g, '_');
		const toStr = (param: object|string, addDefault = false) => (typeof param === 'string')?
				fixStr(param) : fixStr(Object.keys(param)[0])+(addDefault? getDefaultStr(param) : '');
		const toParams = (str, v: object|string, i, arr) => str+toStr(v, true)+(i === (arr.length-1)? '' : ', '); // TODO: add default
		const toArray = (str, v: object|string, i, arr) => str+toStr(v)+(i === (arr.length-1)? '' : ', ');
		return method+'('+params.reduce(toParams, '')+'): Observable<any> { return this.call(\''+method.toLowerCase()
				+'\', ['+params.reduce(toArray, '')+']); }';
	}

	call(method, args?): Observable<any> {
		// console.log('getIfno::: ', arguments.callee.toString() );
		const params: any[] = args? Array.isArray(args)? args: this.parseParams(method, args): [];
		const payload = {method: method, params: params, id: 1, jsonrpc: '2.0'};
	...
	*/
	...
	appendRawChange(hexstring, address, native_fee?): Observable<any> { return this.call('appendrawchange', [hexstring, address, native_fee]); }
	...
	listStreams(stream = '*', verbose = 'false', count?, start?): Observable<any> { return this.call('liststreams', [stream, verbose, count, start]); }
